...
export default function BarcodeDetails({
  ...
  const getCustomerValues = (customer) =>
    omit(customer, [...defaultFeilds, "name", "customerId"]);
  ...
  const getDetails = async () => {
    ...
    if (res && res.status === "success") {
      ...
      setDetails(values);
      setProductDetail && setProductDetail(values);
      startTransition(false);
    } else {
      startTransition(false);
  ...
  const setNewCustomerDetails = (details, type = "fetch") => {
    const cvalues =
      type === "fetch" ? getValues(details) : getCustomerValues(details);
    setCustomer(cvalues);
    setCustomerDetail && setCustomerDetail({ ...cvalues, name: details.name });
        ...
        <Box p={4} mb={5}>
                ...
                >
                  ...
                  {product &&
                    ...
                    JSON.parse(product.product_images) && (
                      <ShowProductImgs
                        imgs={JSON.parse(product.product_images)}
                      />
                    )}
                </VStack>
              ))}
          ...
          {details &&
            ...
            JSON.parse(details.product_images) && (
              <ShowProductImgs imgs={JSON.parse(details.product_images)} />
            )}
          {setData && (
            <Center mt={5}>
              <NxtButton
